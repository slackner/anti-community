...
int main(int argc, char *argv[])
{
  ...
  // Perform the Monte Carlo
  fprintf(stderr, "==> now determining target k <==\n");

  int *best_k = malloc(G.nvertices*sizeof(int));
  for (int i=0; i < G.nvertices; i++) best_k[i] = 0;
  ...
  // Perform the Monte Carlo
  fprintf(stderr, "==> now sampling label vectors <==\n");

  int *best_g = malloc(G.nvertices*G.nvertices*sizeof(int));
  for (int i=0; i < G.nvertices*G.nvertices; i++) best_g[i] = 0;
  ...
  entropy -= pr*log(pr);
      ...
      if (k == target_k)
      {
        for (int i = 0; i < G.nvertices; i++)
        {
          assert(g[i] < G.nvertices);
          best_g[i*G.nvertices + g[i]]++;
        }
      }
    }
  }
  ...
  for (int i=0; i < G.nvertices; i++)
  {
    ...
    {
      if (best_g[i*G.nvertices + j] > counter)
      {
        target_r = j;
        counter = best_g[i*G.nvertices + j];
      }
    }
